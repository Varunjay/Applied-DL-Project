...
def to_device(text, bert_prob = None, real_label = None):
    """
...
class Baseline(Logger):
    ...
    def trainmodel(self, model, train_dataset, test_dataset, output_dir):
        ...
        for epoch in range(epochs):
            ...
            if eval_loss < best_eval_loss or eval_acc > best_acc:
    ...
    def predict(self, model, text_field, text):
        ...
        Args:
            model: model to use
            text_field: text field to create the model input
        ...
        # pad the text
        text_pad = pad_sequence(text, self.config['max_seq_length'])

        # Get indexes of padded sequences
        text_index = to_index_predict(text_field.vocab, text_pad)
        text_index = torch.LongTensor([text_index])

        # Predict the label
        model.eval()
        with torch.no_grad():
            text_index, _, _ = to_device(text_index)
            output = model(text_index.t()).squeeze(1)
            probs = torch.softmax(output, dim=1)
            preds = torch.argmax(probs, dim=1)
            pred = preds.item()
    ...
    def loss(self, output, bert_prob, real_label):
        ...
        Args:
            ...
            bert_prob: probability of the bert model output
            real_label: real label of the text
        """
        raise NotImplementedError()
