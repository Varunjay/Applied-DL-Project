...
class BERTBaseline(Logger):
    ...
    def eval_epoch(self, model, dataset, epoch):
        """
        Evaluate on test set for one epoch
        Args:
            model: model
            dataset: dataset
            epoch: epoch number
    ...
    def load_model(self, output_dir):
        ...
        model.load_state_dict(torch.load(os.path.join(output_dir, self.weights_name), map_location=torch.device('cpu')))
        return model
